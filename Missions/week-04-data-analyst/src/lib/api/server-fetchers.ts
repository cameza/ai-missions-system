  ...
  return [
    {
      ...
      transferType: 'Permanent' as any,
      transferValueUsd: 180000000,
      transferValueDisplay: '€180M',
      status: 'done' as any,
      transferDate: new Date('2025-01-19'),
      window: '2025-winter' as any,
      apiTransferId: 12345,
      createdAt: new Date(),
      updatedAt: new Date(),
    },
...
function transformDatabaseTransfer(db: DatabaseTransfer): Transfer {
  return {
    id: db.id,
    playerId: db.player_id || 0,
    playerFirstName: db.player_first_name,
    playerLastName: db.player_last_name,
    playerFullName: db.player_full_name || `${db.player_first_name} ${db.player_last_name}`,
    age: db.age || undefined,
    position: db.position as any,
    nationality: db.nationality || undefined,
    fromClubId: db.from_club_id || undefined,
    toClubId: db.to_club_id || undefined,
    fromClubName: db.from_club_name,
    toClubName: db.to_club_name,
    leagueId: db.league_id || undefined,
    leagueName: db.league_name,
    transferType: db.transfer_type as any,
    transferValueUsd: db.transfer_value_usd || undefined,
    transferValueDisplay: db.transfer_value_display,
    status: db.status as any,
    transferDate: new Date(db.transfer_date),
    window: db.window as any,
    apiTransferId: db.api_transfer_id,
